...
class PresenceService {
  constructor(io, redisService) {
    // Validate io parameter
    if (!io || typeof io.emit !== 'function') {
      logger.error('Socket.IO instance is not valid. PresenceService requires a valid Socket.IO instance.');
      // Create a mock io object to prevent crashes
      io = {
        emit: () => { logger.warn('Socket.IO not initialized, cannot emit'); },
        to: () => ({ emit: () => { logger.warn('Socket.IO not initialized, cannot emit to room'); } })
      };
    }
    ...
    // Bind all methods that use this.io or are called from async contexts
  ...
  async setupRedisSubscriptionWithRetry() {
    ...
    while (retries < maxRetries) {
      try {
        ...
        retries++;
        logger.warn(
          `Redis subscription setup failed (attempt ${retries}/${maxRetries}):`,
          error.message,
        );
  ...
  async setupRedisSubscription() {
    ...
    if (
      ...
      typeof this.redisService.subscribe !== "function"
    ) {
  ...
  startPeriodicTasks() {
    ...
    setInterval(() => {
      try {
        ...
        logger.error("Error in cleanupStalePresence:", error);
      }
    }, 60000);
    ...
    setInterval(() => {
      try {
        ...
        logger.error("Error in flushDbUpdates:", error);
      }
  ...
  // ========== METHODS REQUIRED BY PRESENCE CONTROLLER ==========

  // For controller's batchUpdatePresence method
  async setUserOffline(userId) {
    return await this.markUserOffline(userId);
  }

  // For controller's clearPresenceCache method
  ...
  getMetrics() {
    ...
    for (const [userId, sockets] of this.onlineUsers.entries()) {
      ...
      // Note: To get actual user types, we'd need to store them
      // For now, just track count per user
  ...
  async userConnected(userId, socketId, userData = {}) {
    try {
      ...
      if (user.accountStatus !== "active") {
        logger.warn(
          `User ${userId} is not active, account status: ${user.accountStatus}`,
        );
        return false;
      }
      ...
      if (userSockets.size === 1) {
        ...
        logger.info(
          `User ${userId} (${user.userType}) came online (socket: ${socketId})`,
        );
      } else {
        logger.debug(
          `User ${userId} added socket ${socketId}, total sockets: ${userSockets.size}`,
        );
      }
    ...
    try {
      ...
      if (this.isRedisAvailable() && this.redisService.getUserPresence) {
        ...
        if (presence) {
          ...
          const isOnline =
            lastSeen > fiveMinutesAgo && presence.status === "online";
  ...
  async getOnlineUsers(limit = 100, offset = 0, userType = "all") {
    try {
      ...
      if (this.isRedisAvailable() && this.redisService.getOnlineUsers) {
        const redisUsers = await this.redisService.getOnlineUsers(
          limit,
          offset,
          userType,
        );
      ...
      const users = await BaseUser.find(query)
        .select(
          "_id firstName lastName avatar userName userType role presence location",
        )
        .limit(limit)
        .skip(offset)
  ...
  async getNearbyOnlineUsers(userId, radiusKm = 10, limit = 50) {
    try {
      ...
      const nearbyUsers = await DatingUser.find({
        ...
        .select(
          "_id firstName lastName avatar userName age bio interests location presence",
        )
        .limit(limit);
  ...
  broadcastUserStatus(userId, status, user = null) {
    // Add null check for this.io
    if (!this.io || typeof this.io.emit !== 'function') {
      logger.error(`Cannot broadcast status: Socket.IO instance not available (userId: ${userId}, status: ${status})`);
      return;
    }
    ...
    try {
      ...
      for (const user of staleUsers) {
        ...
        if (
          ...
          await this.markUserOffline(userId);
          logger.info(
            `Cleaned up stale presence for user ${userId} (${user.userType})`,
          );
        }
      }
    } catch (error) {
