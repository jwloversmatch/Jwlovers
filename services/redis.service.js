  ...
  async getOfflineMessages(userId) {
    return this.safeOperation(
      async () => {
        ...
        if (!lockAcquired) {
          this.logger.warn(
            `Lock not acquired for offline messages of user ${userId}`,
          );
          return [];
        }
  ...
  async isDuplicateMessage(senderId, receiverId, content, windowMs = 2000) {
    ...
    return this.safeOperation(
      async () => {
        ...
        if (exists) {
          this.logger.warn(
            `⏭️ Duplicate message blocked: ${senderId} -> ${receiverId}`,
          );
          return true;
        }
  ...
  async getOnlineUsers(limit = 100) {
    return this.safeOperation(
      async () => {
        ...
        const userIds = await this.pubClient.zrange(
          onlineSetKey,
          0,
          limit - 1,
          "REV",
    ...
    return this.safeOperation(
      async () => {
        ...
        const currentConnections =
          (await this.pubClient.get(connectionsKey)) || 0;
