...
const getDatingUserWithProfile = async (userId) => {
  return await DatingUser.findById(userId)
    .populate({
      path: "profile",
      select:
        "basic photos location faith relationship career lifestyle personality preferences badges stats progress settings",
    })
...
const calculateCompatibilityWithoutSession = async (user1Id, user2Id) => {
  try {
    ...
    if (basicPref1.ageRange && age2 != null) {
      if (age2 < basicPref1.ageRange.min || age2 > basicPref1.ageRange.max)
        return 0;
    }
    if (basicPref2.ageRange && age1 != null) {
      if (age1 < basicPref2.ageRange.min || age1 > basicPref2.ageRange.max)
        return 0;
    }
    ...
    if (
      db1.includes("smoking") &&
      p2.lifestyle?.smoking &&
      p2.lifestyle.smoking !== "never"
    )
      return 0;
    if (
      db1.includes("drinking") &&
      p2.lifestyle?.drinking &&
      p2.lifestyle.drinking !== "never"
    )
      return 0;
    if (
      db2.includes("smoking") &&
      p1.lifestyle?.smoking &&
      p1.lifestyle.smoking !== "never"
    )
      return 0;
    if (
      db2.includes("drinking") &&
      p1.lifestyle?.drinking &&
      p1.lifestyle.drinking !== "never"
    )
      return 0;
    ...
    if (faithPref1.length === 0 || faithPref1.includes(p2.faith?.servingAs))
      faithPts += 5;
    if (faithPref2.length === 0 || faithPref2.includes(p1.faith?.servingAs))
      faithPts += 5;
    ...
    if (
      p1.preferences?.faith?.missionaryPreferred &&
      p2.faith?.missionary?.served
    )
      faithPts += 2;
    if (
      p2.preferences?.faith?.missionaryPreferred &&
      p1.faith?.missionary?.served
    )
      faithPts += 2;

    if (p1.preferences?.faith?.bethelPreferred && p2.faith?.bethel?.served)
      faithPts += 2;
    if (p2.preferences?.faith?.bethelPreferred && p1.faith?.bethel?.served)
      faithPts += 2;

    if (
      p1.preferences?.faith?.mustBeJW &&
      p2.preferences?.faith?.mustBeJW &&
      p1.faith?.servingAs &&
      p2.faith?.servingAs
    ) {
      faithPts += 1;
...
exports.findMatches = async (req, res) => {
  try {
    ...
    if (!currentUser?.profile) {
      return res
        .status(404)
        .json({ success: false, error: "Dating profile not found" });
    }

    if (premium === "true" && !currentUser.isPremium) {
      return res
        .status(403)
        .json({ success: false, error: "Premium feature. Upgrade to access." });
    }
    ...
    const myBlockedUserIds = (
      currentUser.datingPrivacySettings?.hideProfileFrom || []
    ).map((id) => id.toString());
    ...
    if (excludedProfileIds.length > 0) {
      profileQuery._id = {
        $nin: excludedProfileIds.map((id) => new mongoose.Types.ObjectId(id)),
      };
    }
    ...
    if (myPrefs.faith?.missionaryPreferred)
      profileQuery["faith.missionary.served"] = true;
    if (myPrefs.faith?.bethelPreferred)
      profileQuery["faith.bethel.served"] = true;
    ...
    if (premium === "true") {
      ...
      if (req.query.minHeight || req.query.maxHeight) {
        ...
        if (req.query.minHeight)
          profileQuery["basic.height"].$gte = parseInt(req.query.minHeight);
        if (req.query.maxHeight)
          profileQuery["basic.height"].$lte = parseInt(req.query.maxHeight);
      }
    }

    const candidateProfiles = await Profile.find(profileQuery)
      .select(
        "userId basic photos location faith relationship career lifestyle personality preferences badges stats progress settings",
      )
      .lean();
    ...
    const datingUsersArr = await DatingUser.find({
      ...
      .select(
        "_id isPremium boost incognitoMode travelMode presence datingPrivacySettings",
      )
      .lean();

    const datingUserMap = new Map(
      datingUsersArr.map((u) => [u._id.toString(), u]),
    );
    ...
    joined.sort((a, b) => {
      const aBoost = !!(
        a.du.boost?.isActive && new Date(a.du.boost.expiresAt) > new Date()
      );
      const bBoost = !!(
        b.du.boost?.isActive && new Date(b.du.boost.expiresAt) > new Date()
      );
      if (bBoost !== aBoost) return bBoost ? 1 : -1;
      if (b.du.isPremium !== a.du.isPremium) return b.du.isPremium ? 1 : -1;
      const aLast = a.profile.stats?.lastActive
        ? new Date(a.profile.stats.lastActive).getTime()
        : 0;
      const bLast = b.profile.stats?.lastActive
        ? new Date(b.profile.stats.lastActive).getTime()
        : 0;
      if (bLast !== aLast) return bLast - aLast;
      return (
        (b.profile.progress?.completion || 0) -
        (a.profile.progress?.completion || 0)
      );
    });

    const total = joined.length;
    const paged = joined.slice(
      parseInt(skip),
    ...
    const matchesWithScores = await Promise.all(
      paged.map(async ({ profile, du }) => {
        const compatibility = await calculateCompatibilityWithoutSession(
          userId,
          du._id,
        );
        ...
        return {
          user: {
            ...
            isBoostActive: !!(
              du.boost?.isActive && new Date(du.boost.expiresAt) > new Date()
            ),
          },
          compatibility,
          distance,
...
exports.likeUser = async (req, res) => {
  ...
  try {
    ...
    if (userId === targetUserId) {
      return res
        .status(400)
        .json({ success: false, error: "Cannot like yourself" });
    }
    ...
    if (!currentUser?.profile) {
      return res
        .status(404)
        .json({ success: false, error: "Your dating profile not found" });
    }

    if (!targetUser?.profile) {
      return res
        .status(404)
        .json({ success: false, error: "Target user profile not found" });
    }
    ...
    if (alreadyLiked) {
      return res
        .status(400)
        .json({ success: false, error: "Already liked this user" });
    }
    ...
    currentUser.datingStats.totalLikes =
      (currentUser.datingStats.totalLikes || 0) + 1;
    ...
    if (isMutualLike) {
      ...
      currentUser.datingStats.totalMatches =
        (currentUser.datingStats.totalMatches || 0) + 1;
      targetUser.datingStats = targetUser.datingStats || {};
      targetUser.datingStats.totalMatches =
        (targetUser.datingStats.totalMatches || 0) + 1;
...
exports.passUser = async (req, res) => {
  try {
    ...
    const targetUser =
      await DatingUser.findById(targetUserId).populate("profile");
    ...
    if (permanent) {
      currentUser.datingPrivacySettings =
        currentUser.datingPrivacySettings || {};
      currentUser.datingPrivacySettings.hideProfileFrom =
        currentUser.datingPrivacySettings.hideProfileFrom || [];
...
exports.undoLastAction = async (req, res) => {
  try {
    ...
    if (!currentUser.likedProfiles?.length) {
      return res
        .status(400)
        .json({ success: false, error: "No recent actions to undo" });
    }

    const lastLike = currentUser.likedProfiles.pop();

    if (lastLike.profile) {
      const profileIdToRemove = (
        lastLike.profile._id ?? lastLike.profile
      ).toString();
    ...
    return res
      .status(500)
      .json({ success: false, error: "Unable to undo action" });
  }
};
  ...
  try {
    ...
    const matches = await Match.find({ users: userId, status })
      .populate({
        ...
        populate: {
          path: "profile",
          select: "basic photos location badges stats",
        },
      })
      .sort({ matchedAt: -1 })
      .skip(parseInt(skip))
    ...
    return res
      .status(500)
      .json({ success: false, error: "Unable to get matches" });
  }
};
...
exports.getMutualLikes = async (req, res) => {
  try {
    ...
    const usersWhoLikedMe = await DatingUser.find({
    ...
    for (const user of usersWhoLikedMe) {
      if (
        user.profile &&
        currentUserLikes.includes(user.profile._id.toString())
      ) {
    ...
    return res
      .status(500)
      .json({ success: false, error: "Unable to get mutual likes" });
  }
};
...
exports.getWhoLikedMe = async (req, res) => {
  try {
    ...
    const likes = usersWhoLikedMe.map((user) => ({
      ...
      likedAt: user.likedProfiles?.find(
        (lp) => lp.profile?.toString() === currentUser.profile._id.toString(),
      )?.likedAt,
    }));
    ...
    return res
      .status(500)
      .json({ success: false, error: "Unable to get likes" });
  }
};
...
exports.getMyLikedUserIds = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ success: false, error: "Unable to get liked users" });
  }
};
...
exports.getMatchById = async (req, res) => {
  try {
    ...
    const match = await Match.findById(matchId).populate({
      ...
      populate: {
        path: "profile",
        select: "basic photos location faith badges",
      },
    });
    ...
    return res
      .status(500)
      .json({ success: false, error: "Unable to get match" });
  }
};
...
exports.blockUser = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ success: false, error: "Unable to block user" });
  }
};
...
exports.reportMatch = async (req, res) => {
  try {
    ...
    match.metrics.reported = match.metrics.reported || {
      count: 0,
      reasons: [],
    };
    ...
    return res
      .status(500)
      .json({ success: false, error: "Unable to report match" });
  }
};
...
exports.archiveMatch = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ success: false, error: "Unable to archive match" });
  }
};
...
exports.getMatchStats = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ success: false, error: "Unable to get match statistics" });
  }
};
...
exports.getMatchInsights = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ success: false, error: "Unable to get insights" });
  }
};
...
exports.getCompatibilityReport = async (req, res) => {
  try {
    const { userId: targetUserId } = req.params;
    const userId = req.user?.id || req.userId;
    const score = await calculateCompatibilityWithoutSession(
      userId,
      targetUserId,
    );
    ...
    return res
      .status(500)
      .json({ success: false, error: "Unable to get compatibility report" });
  }
};
...
exports.updateMatchPreferences = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ success: false, error: "Unable to update preferences" });
  }
};
...
exports.getMatchPreferences = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ success: false, error: "Unable to get preferences" });
  }
};
...
exports.resetMatchPreferences = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ success: false, error: "Unable to reset preferences" });
  }
};
...
exports.quickLikeMultiple = async (req, res) => {
  try {
    ...
    if (!Array.isArray(userIds) || userIds.length === 0) {
      return res
        .status(400)
        .json({ success: false, error: "Please provide an array of user IDs" });
    }

    const currentUser =
      await DatingUser.findById(currentUserId).populate("profile");
    const results = [];
    ...
    return res
      .status(500)
      .json({ success: false, error: "Unable to process quick like" });
  }
};
...
exports.quickPassMultiple = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ success: false, error: "Unable to process quick pass" });
  }
};
...
exports.refreshMatchQueue = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ success: false, error: "Unable to refresh queue" });
  }
};
...
exports.clearSeenProfiles = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ success: false, error: "Unable to clear seen profiles" });
  }
};
...
exports.handleMatchWebhook = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ success: false, error: "Unable to process webhook" });
  }
};
...
exports.getDiagnostics = async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ success: false, error: "Unable to get diagnostics" });
  }
};
...
Object.keys(exports).forEach((k, i) =>
