...
const adminOptionsLimiter = createDynamicRateLimiter({
  ...
  keyGenerator: (req) => {
    ...
    return userId ? `admin:options:${operation}:user:${userId}` : `admin:options:${operation}:ip:${req.ip}`;
  },
...
const securityQuestionsLimiter = createDynamicRateLimiter({
  ...
  keyGenerator: (req) => {
    const userId = req.user?.id;
    const operation = req.method.toLowerCase();
    return userId ? `admin:security:${operation}:user:${userId}` : `admin:security:${operation}:ip:${req.ip}`;
  },
...
const importExportLimiter = createDynamicRateLimiter({
  ...
  keyGenerator: (req) => {
    const userId = req.user?.id;
    return userId ? `admin:importExport:user:${userId}` : `admin:importExport:ip:${req.ip}`;
  },
  message: 'Too many import/export operations. Please wait before trying again.'
});
...
router.get('/dashboard', 
  ...
  (req, res, next) => {
    ...
    const userData = global.rateLimitStore.get(key) || { count: 0, resetTime: now + windowMs };
...
router.get('/users/type/:userType',
  requireMinimumRole(ROLES.ADMIN),
  [
    param('userType')
...
router.get('/security-questions/:id',
  requireMinimumRole(ROLES.ADMIN),
  [
    param('id')
      .isMongoId()
...
router.put('/security-questions/:id',
  requireMinimumRole(ROLES.ADMIN),
  [
    param('id')
      .isMongoId()
...
router.patch('/security-questions/:id/toggle',
  requireMinimumRole(ROLES.ADMIN),
  [
    param('id')
      .isMongoId()
...
router.delete('/security-questions/:id',
  requireMinimumRole(ROLES.ADMIN),
  [
    param('id')
      .isMongoId()
...
router.get('/system/database/backup',
  superAdminOnly,
...
router.get('/roles/:role/users',
  ...
  [
    ...
    (req, res, next) => {
      ...
      if (req.params.role === ROLES.SUPER_ADMIN && req.user.role !== ROLES.SUPER_ADMIN) {
      ...
      if (req.params.role === ROLES.ADMIN && req.user.role !== ROLES.ADMIN && req.user.role !== ROLES.SUPER_ADMIN) {
        return res.status(403).json({
          success: false,
  ...
  async (req, res) => {
    try {
      ...
      const recentActivity = await BaseUser.find({
        ...
        .limit(10)
        .select('firstName lastName email userType role presence.lastSeen department');
...
router.use((err, req, res, next) => {
  ...
  if (err.message?.includes('user type') || err.message?.includes('DatingUser') || err.message?.includes('staff')) {
    return res.status(403).json({
      success: false,
      error: err.message,
